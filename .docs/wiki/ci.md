...
3. **Restore** — `dotnet restore WealthOps.slnx`.
...
5. **Aspire test with coverage** — local action `.github/actions/aspire-test-with-coverage`:
   - Starts `tests/WealthOps.TestFramework.Aspire`, keeps its PID inside the action script, and waits for PostgreSQL (`127.0.0.1:15432`), Redis (`127.0.0.1:16379`), and WireMock (`http://127.0.0.1:19091/__admin/health`).
