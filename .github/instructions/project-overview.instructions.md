...
| API style | Minimal API endpoints in `src/WealthOps.Host` |
...
```
src/
  WealthOps.Domain/          # Entities, value objects, invariants
  WealthOps.Application/     # Feature slices + Mediator handlers/pipelines
  WealthOps.Infrastructure/  # EF Core persistence + external integrations
  WealthOps.Host/            # Minimal API composition, middleware, observability

tests/
  WealthOps.*.UnitTest/
  WealthOps.*.ComponentTest/
  WealthOps.*.IntegrationTest/
  WealthOps.TestFramework/
```
