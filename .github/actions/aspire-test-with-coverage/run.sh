...
coverage_collect="XPlat Code Coverage;Format=cobertura;Include=[WealthOps.*]*;ExcludeByFile=**/*.g.cs,**/obj/**,**/Migrations/*.cs,**/*ModelSnapshot.cs"
aspire_pid=""
...
dotnet run \
  --project tests/WealthOps.TestFramework.Aspire \
  --configuration "${build_configuration}" \
  --no-build &
...
run_test_project tests/WealthOps.Host.IntegrationTest/WealthOps.Host.IntegrationTest.csproj \
  || record_failure "Host integration tests failed."
ensure_aspire_alive

echo "Phase 2 — component tests (parallel)..."
run_test_project tests/WealthOps.Application.ComponentTest/WealthOps.Application.ComponentTest.csproj &
app_component_pid=$!
run_test_project tests/WealthOps.Infrastructure.ComponentTest/WealthOps.Infrastructure.ComponentTest.csproj &
infra_component_pid=$!
...
run_test_project tests/WealthOps.Domain.UnitTest/WealthOps.Domain.UnitTest.csproj &
domain_unit_pid=$!
run_test_project tests/WealthOps.Application.UnitTest/WealthOps.Application.UnitTest.csproj &
app_unit_pid=$!
run_test_project tests/WealthOps.Infrastructure.UnitTest/WealthOps.Infrastructure.UnitTest.csproj &
infra_unit_pid=$!
run_test_project tests/WealthOps.Host.UnitTest/WealthOps.Host.UnitTest.csproj &
host_unit_pid=$!
