...
- `classify_horizontal_slice()` keys the `backend` layer off path prefixes `src/` and `WealthOps` — if the solution is renamed again, that prefix list must be updated or backend changes silently classify as `general`.
...
| 2026-09-13 | Repo renamed from placeholder `Project` to `WealthOps`; updated `classify_horizontal_slice()` path prefix accordingly. | |
