...
| 2026-09-20 | Added the `create-worktask` skill (replaces the Claude-only `worktask-create.sh` hook). Its `assets/WORKTASK.template.md` deliberately carries **no HTML comments**, so it needs no new `P2` "Hidden Instructions" baseline entry — unlike `create-hld/assets/*`, which holds four. | |
