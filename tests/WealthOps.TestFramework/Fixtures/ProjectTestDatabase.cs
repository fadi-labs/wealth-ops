...
public sealed class WealthOpsTestDatabase : IAsyncDisposable
{
    ...
    private WealthOpsTestDatabase(string connectionString, string databaseName, string maintenanceConnectionString)
    {
        ...
        ITestOutputHelper? output = null)
    {
        ...
        // services.AddDbContext<WealthOpsDbContext>(opts => opts.UseNpgsql(connectionString));
        // await using var provider = services.BuildServiceProvider();
        // await provider.MigrateWealthOpsAsync(cancellationToken);

        Log(output, connectionString, $"Database '{databaseName}' ready.");

        return new WealthOpsTestDatabase(connectionString, databaseName, maintenanceConnectionString);
    }
